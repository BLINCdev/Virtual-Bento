...
/*
Purpose of this script is to be able to access the unity main thread from other threads.
Executable actions are added to the unity main thread for the main thread to execute.
See example usage of this function in the brachIOplexusInputHandler class.

IMPORTANT NOTES: 
* It is recommended to only add actions to the main thread that are REQUIRED to be executed on the main thread (e.g. actions that need to access a game object).
* It is also possible one action may inturrupt another action, especially during the unity initialization process (e.g. when loading a new scene). 
* Best to make sure that scene is already loaded and objects are in place before using this class.
